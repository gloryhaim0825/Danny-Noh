...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateAdminUI() {
    if (isAdmin) {
      ...
      if (adminStatusText) adminStatusText.textContent = '선교사 로그인';
  ...
  // --- Universal Cross-Device Copy Function (Smartphones, iOS Safari, KakaoTalk/In-App Browsers, Desktop) ---
  function copyTextToClipboard(text, successToastMsg) {
    let copiedSuccess = false;

    // 1. Synchronous execCommand attempt first to ensure user activation token is active (iOS Safari / In-App Browsers)
    try {
      const textarea = document.createElement('textarea');
      textarea.value = text;
      textarea.setAttribute('readonly', '');
      // Place in viewport with opacity 0.001 so iOS Safari / WebKit doesn't reject off-screen elements
      ...
      // Explicit selection range for iOS Safari
    ...
    if (!copiedSuccess && navigator.clipboard && window.isSecureContext) {
      return navigator.clipboard.writeText(text).then(() => {
        showToast(successToastMsg || '계좌번호가 복사되었습니다!');
        return true;
      }).catch(err => {
        console.error('navigator.clipboard failed:', err);
        prompt('계좌번호를 복사해 주세요:', text);
        return false;
      });
    }
    ...
    if (copiedSuccess) {
      ...
      prompt('아래 계좌번호를 길게 눌러 복사해 주세요:', text);
  ...
  // 1. Account Number Only Button Click
  if (btnCopyAccount) {
  ...
  // 2. Full Account Info Button Click
  ...
  // 3. Tapping directly on Account Number Text
  if (accountNumText) {
  ...
  // 4. Tapping anywhere on the Account Box Card
  if (accountBoxCard) {
  ...
  // 5. Footer Copy Button & Text Click
  if (btnFooterCopy) {
